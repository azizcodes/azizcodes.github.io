...
One is confronted with a large set of options for the `rsync` command for the simple need of copying something *correctly*. To make things easy, I'll consider the two scenarios below. See [usage examples](https://rsync.samba.org/examples.html).
...
## Scenario 2: Mirroring 

To copy and replace old versions of the same file.

``` bash
rsyc -av --delete source destination
```
